...
public class Inventario {
    ...
    private String url = "jdbc:mysql://localhost:3306/java_jdbc";  //String de conexion
    private String user = "root";
    private String password = "sasa";

    public Inventario() {
        this.productos = new ArrayList<>(); //contructor,para inicializar lista de productos
    }
    ...
    public void mostrarProductosDesdeBaseDeDatos() {
        try (Connection conn = DriverManager.getConnection(url, user, password)) {
            ...
            if (!resultado.isBeforeFirst()) {        //si en la base de datos no hay productos
            ...
            System.out.println("Error al mostrar productos desde la base de datos: " + e.getMessage());
                                            ...
                                            // Metodo para eliminar directamente un producto por nombre de la base
    public void eliminarProductoPorNombre(String nombre) {
        ...
        try (Connection conn = DriverManager.getConnection(url, user, password)) {
            ...
            if (rowsAffected > 0) {
                System.out.println("Producto con nombre '" + nombre + "' eliminado de la base de datos.");
            }
    ...
    public void sincronizarConBaseDeDatos() {
        try (Connection conn = DriverManager.getConnection(url, user, password)) {
            for (Producto producto : productos) {
                ...
                if (resultado.next()) {
                                                        // Si el producto existe entonces  actualizamos la cantidad
            ...
            System.out.println("Error al sincronizar con la base de datos: " + e.getMessage());
                                            ...
                                            // Metodo para buscar un producto por nombre en la lista
